...
browser.setTreeData = function(data, path) {
    ...
    if (typeof(special) === 'undefined')
     {
      special = get_special(); // #Chrome EXT#Documents# ecc...
      //alert('special');
     } 
    if (typeof mySite === 'undefined')
     MySite = get_MySite(); // #mysite# ecc...
    if (typeof Public === 'undefined')
     Public = get_Public(); // #mysite# ecc...

    
    
    if (special.indexOf('#'+data.name+'#')>-1)
     $(selector + ' span.folder').addClass(data.current ? 'current special' : 'regular special');

    if (MySite.indexOf('#'+data.name+'#')>-1)
     $(selector + ' span.folder').addClass(data.current ? 'current mysite' : 'regular mysite');

    if (Public.indexOf('#'+path+'#')>-1)
     $(selector + ' span.folder').addClass(data.current ? 'current Public' : 'regular Public');
...
browser.buildTree = function(root, path) {
    ...
    html += '</div>';
    $('#folders_action').html("<br />&nbsp;<button onclick=nuova_cartella('files')>" + this.label("New Subfolder...") + "</button>");
...
browser.expandDir = function(dir) {
    ...
    $('#folders_action').html("<br />&nbsp;<button onclick=nuova_cartella('" + path + "')>" + this.label("New Subfolder...") + "</button>");
...
browser.changeDir = function(dir) {
    if (dir.children('span.folder').hasClass('regular')) {
		...
		$('#folders_action').html("<br />&nbsp;<button onclick=nuova_cartella('" + dir.data('path') + "')>Nuova Cartella...</button>");
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var title = "Crypt2Share.com: /" + browser.dir;
...
browser.menuDir = function(dir, e) {
	...
	html += '<a href="kcact:public"' + (!data.removable ? ' class="denied"' : '') + '>' +
		this.label("Public Upload") + '</a>';
    html += '</div>';
    ...
    $('.menu a[href="kcact:public"]').click(function() {
		create_public_upload(data.path);
    });
...
browser.refreshDir = function(dir) {
    ...
    $('#folders_action').html("<br />&nbsp;<button onclick=nuova_cartella('" + path + "')>Nuova Cartella...</button>");
