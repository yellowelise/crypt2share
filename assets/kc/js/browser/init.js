...
browser.resize = function() {
    _('left').style.width = '20%';
    _('right').style.width = '80%';
    ...
    if (document.getElementById("secure").style.visibility == "visible")
     {
		_('left').style.height = _('right').style.height =
			$(window).height() - $('#status').outerHeight() - 210 + 'px'; // 25% per secure!!!
     }
    else
     {
		_('left').style.height = _('right').style.height =
			$(window).height() - $('#status').outerHeight()  + 'px'; // 25% per secure!!!
	 } 
    _('folders').style.height = $('#left').outerHeight() - _.outerVSpace('#folders') - 45 + 'px';
