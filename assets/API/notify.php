...
function notify($idto,$from,$title, $message,$ut,$filename)
{
 if ($from != '')
 ...
 if ($db1->Error()) $db1->Kill();
 $db1->Open();
 $sql = "insert into notification (iduser,title,text,from_not,uploadticket,file_for_you) values ('".$idto."','".$title."','".$message."','".$from."','".$ut."','".$filename."')";
 //echo $sql."<br />";
 ...
 notify($id,$_SESSION['iduser'],"Un file criptato per te!",filename_from_ut($ut) . "&nbsp;<button onclick=shared_decrypt(\'".$ut."\')>Decripta e copia</button>",$ut,filename_from_ut($ut));
