...
function get_images($d,$http)
{
	$ud = $d;
	$d = $_SESSION["path"] . $d;
	$images = glob($d."{*.jpeg,*.jpg,*.gif,*.png}", GLOB_BRACE);
    $quante = count($images);
    ...
    for ($i = 0;$i<$quante;$i++)
     {
		...
		if ((($i % 6) === 0)||($i==0)) 
		 $res .= "<div class='row-fluid'>";
          ...
          if ($files[$i])
           {
			   $res .= "<div class='span2'><a  href='".$http.$ud.basename($files[$i])."'><img alt='".$i."' src='../../img_my.php?fn=".$ud . basename($files[$i])."' ><br /></a>".basename($files[$i])."</div>";
		   }		 
...
function get_video($d,$http)
{
	...
	$ud = $d;
	$d = $_SESSION["path"] . $d;
	$videos = glob($d."{*.mp4,*.3gp,*.flv,*.mov}", GLOB_BRACE);
    $quante = count($videos);
    ...
    for ($i = 0;$i<$quante;$i++)
     {
		...
		if ((($i % 4) === 0)||($i==0)) 
		 $res .= "<div class='row-fluid'>";
          ...
          if ($videos[$i])
           {
			   $res .= "<div class='span3'><iframe frameborder=0  style='width:330px;height:250px;overflow:hidden;' src='../../video/play_my.php?f=".$ud . basename($videos[$i])."&w=320&h=240'></iframe><br />".basename($videos[$i])."</div>";
		   }		 
...
<head>
  ...
  <meta name="author" content="">
	...
	<link href="../../mysite_template/1/css/style.css" rel="stylesheet">
...
<div class="navbar navbar-fixed-bottom" style="height:80px;">
	<div class="row-fluid" style="background:#333;color:#fff">
		<div class="span12">
			<div class="row-fluid">
				<div class="span6">
					 <img src="<?php echo $_SESSION['app_address']."images/logook200.png" ?>"><strong>Crypt2Share.com</strong> Unlimited disk space for your files&nbsp;&nbsp;&nbsp;<a href="https://www.crypt2share.com" class="btn">Register for Free</a>
				</div>
				<div class="span6">
