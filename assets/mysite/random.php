...
for ($i = 0;$i<$quante;$i++)
 {
	...
	if ((($i % 4) === 0)||($i==0)) 
	 $res .= "<div class='row-fluid'>";
	  ...
	  if ($users[$i])
	   {
		   $res .= "<div class='span3'><a href='".$_SESSION['mysite_address'].basename($users[$i])."'><h5>MySite: ".basename($users[$i])."</h5></a>";
			 for ($j=0;$j<9;$j++)	
			   {
				if ($immagini[$j])   
				 $res .= "<img src='img_my.php?fn=".urlencode(str_replace( $_SESSION['path'] ,"",$immagini[$j]))."&c=1&s=60'>";
				if (($j==2)||($j==5))
				 $res .="<br />";
			   }	
		   $res .= "</div>";
